...
Functions:
    ...
    tri_analysis(filename, timeseries_flag, timerange, mutations_in_snv_cycles):
        Computes the raw topological recurrence index (raw tRI) and writes the results to a CSV
        file.
...
def retrieve_snv_cycles(in_ripser: str) -> Tuple[List[Cycle], List[int]]:
    ...
    Example:
        ...
        Each interval [b,d) is a persistence interval with two representatives, where
        the second one is the exhaustive representative used by evotrec. The lines under 
        "persistence intervals in dim 1:" are the ones that are processed to extract
        the cycle representatives.
    """
...
if __name__ == "__main__":
    ...
    if args.timeseries is not False:
        ...
        print("Ripser...")
        os.system(
            f"ripser-representatives --dim 1 --format sparse --threshold {timerange+1} "
            f"{filename + '.timedist'} > {filename + '.ripser'}"
        )
    else:
        print("Ripser...")
        os.system(
            f"ripser-representatives --dim 1 --format sparse --threshold 2 "
